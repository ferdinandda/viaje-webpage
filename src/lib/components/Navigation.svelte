...
>
    <nav class="mx-auto max-w-7xl px-[var(--spacing-editorial)] md:px-8 lg:px-12">
      ...
      <div class="hidden md:flex items-center justify-between h-20 md:h-24">
        <!-- Discreet Logo (Left aligned, only on internal pages) -->
        <div class="flex-1">
          {#if !isHomepage}
            <div class="opacity-40 hover:opacity-100 transition-opacity duration-500">
              <Logo {useLightNav} customClass="text-sm tracking-[0.2em]" />
            </div>
          {/if}
        </div>
        ...
        <div class="flex-1">
          {#if !isHomepage}
            <div class="opacity-40">
              <Logo {useLightNav} customClass="text-xs" />
            </div>
          {/if}
        </div>
