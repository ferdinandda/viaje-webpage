...
// ESSAYS
export const getEssayBySlug = query({
  args: { slug: v.string() },
  handler: async (ctx, args) => {
    return await ctx.db
      .query("essays")
...
export const updateEssayAudioUrl = mutation({
  args: { 
    id: v.id("essays"), 
    audioUrl: v.string() 
  },
...
export const seedRun = mutation({
  handler: async (ctx) => {
    ...
    const existing = await ctx.db
      .query("essays")
      .withIndex("by_slug", (q) => q.eq("slug", florenciaEssay.slug))
      .unique();

    if (existing) {
      await ctx.db.patch(existing._id, florenciaEssay);
      return { status: "updated", id: existing._id };
    } else {
      const id = await ctx.db.insert("essays", florenciaEssay);
